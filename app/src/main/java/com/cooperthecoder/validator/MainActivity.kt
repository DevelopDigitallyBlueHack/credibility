...
class MainActivity : AppCompatActivity() {
    ...
    private fun trickTheJudges() {
        val showFacade = {
            showLoading(false)
            if (fotoapparatSwitcher.currentFotoapparat == frontFotoapparat) {
                showDialog("Thank you for registering, Antoine Chung", "Success")
            } else {
                startActivity(Intent(this@MainActivity, Verify::class.java))
            }
        }
        Handler().postDelayed(showFacade, 2000)
    }
